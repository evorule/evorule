...
//! evorule-governance compile-time gate (L1 字面量门禁)
//!
...
//! - 另执行策略层反模式检测 (P1-P4, 与 tcb/reactor 同一份内联实现)
...
fn main() -> ExitCode {
    ...
    if std::env::var("EVORULE_SKIP_CR_GATE").is_ok() {
        println!(
            "cargo:warning={crate_name} change governance gate SKIPPED via EVORULE_SKIP_CR_GATE"
        );
    } else {
        // 执行变更治理门禁验证
    ...
    for path in &files {
        ...
        for (label, needle) in FORBIDDEN {
            ...
            for (lineno, line) in content.lines().enumerate() {
                ...
                // 注意: 此豁免必须与 evorule-reactor/build.rs 保持一致 (双层一致性)
