...
- **`io_request` 是"半个"元指令**:它**不修改任何状态**,只产生一个
  ...
  让上层反应器(evorule-reactor)去执行 I/O,然后把结果注入
...
首次执行（**io_result** 不存在）：
instruction(call_external) → branch(exists(**io_result**)=false) → io_request → IoRequired

恢复执行（**io_result** 已注入）：
instruction(call_external) → branch(exists(**io_result**)=true) → set(llm_response, **io_result**) → State
...
#### [value.rs](file:///d:/evorule/evorule-tcb/src/value.rs) — JsonValue
...
#### [path.rs](file:///d:/evorule/evorule-tcb/src/path.rs) — 路径解析
...
#### [domain.rs](file:///d:/evorule/evorule-tcb/src/domain.rs) — 域评估

- 6 种基本域类型：`eq` / `lt` / `exists` / `instruction` / `all` / `not`
- `MAX_DOMAIN_DEPTH = 64` 防止无限递归
- 派生域类型（`gt`/`ne`/`ge`/`le`/`or`）由 `core_eval.json` 组合实现

#### [executor.rs](file:///d:/evorule/evorule-tcb/src/executor.rs) — 元指令执行器
...
#### [transition.rs](file:///d:/evorule/evorule-tcb/src/transition.rs) — 状态转换入口
...
> **权威源**:`evorule-tcb/build.rs` 现引用本仓库内 SPEC 文档作为
> 14 条 redline (T1-T14) 的源,不再依赖 `文档/` 目录(已 .gitignore)。

实际权威源:

- `evorule-tcb/TCB_SPEC.md` — 本模块 14 条 redline (T1-T14)
