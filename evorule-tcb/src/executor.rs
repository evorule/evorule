...
/// Kani 专用 `resolve_path_or_literal`: 剪枝 `__` 路径分支
///
/// # 设计理由（2026-07-30）
///
/// 生产版 `resolve_path_or_literal` 对 `__` 开头的字符串调用 `resolve_path`。
/// Kani 无法具体化 FixedMap 中的字符串内容,探索 `__` 分支时对**符号路径**
/// 调用 `resolve_path_bytes`,其 `while` 循环对符号 `path.len()` 无界展开,
/// 导致 `verify_execute_transition_kani`(经 `exec_set` 调用)路径爆炸超时。
///
/// 此版本用 `kani::assume(!s.starts_with("__"))` 剪枝 `__` 分支,使 Kani
/// 只探索字面值路径。
///
/// # Soundness
///
/// 对 `verify_execute_transition_kani` 是 sound 的:其 `attr="x"`、`value=42`
/// 均非 `__` 路径引用。`__` 路径解析的正确性由以下 proof 独立覆盖:
/// - `verify_resolve_path_object_kani`:resolve_path 对 Object 的查找
/// - `verify_evaluate_domain_*_kani`:resolve_domain_path 路径解析
///
/// 当前无其他 Kani proof 经 `exec_set`/`exec_push`/`exec_io_request` 使用
/// `__` 路径引用;若未来新增此类 proof,需移除此 assume 或为其单独建模。
#[cfg(kani)]
fn resolve_path_or_literal(
    _state: &JsonValue,
    val: Option<&JsonValue>,
    ...
    match val {
        Some(JsonValue::String(s)) => {
            // 假设非 __ 路径(剪枝符号路径循环爆炸)
            kani::assume(!s.starts_with("__"));
...
pub(crate) fn exec_set(instr: &JsonValue, mut state: JsonValue) -> Result<JsonValue, TcbError> {
    let params = instr
        .get("params")
