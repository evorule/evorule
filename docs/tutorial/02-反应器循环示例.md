...
> v0.6.0 起为**单轮语义**,而非单步的 `execute_transition`(教程 01)。
...
> **v0.6.0 变更**：机制层不再内置 LLM 多轮循环原语（`collect`/`merge` 已随规则清理退役）。
...
> **single-round semantics** since v0.6.0, rather than the single-step `execute_transition` (Tutorial 01).
