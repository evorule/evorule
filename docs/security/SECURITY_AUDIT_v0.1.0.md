...
1. **Zero `unsafe` everywhere** (tier1 `ffi.rs` 局部豁免,见 AGENTS.md): `#![forbid(unsafe_code)]` in every Cargo.toml;tier1-reactor `ffi.rs` 标注 `#![allow(unsafe_code)]` 用于 FFI 边界,已文档化。
