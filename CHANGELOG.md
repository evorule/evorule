...
**TCB Ignored 变体 + I/O 结果按类型隔离 (MINOR 跨 0.3.0)** — 机制层从"静默失败"转"显式 Error 事实";I/O 结果从全局 `__io_result__` 改为按 io_type 隔离的 `__io_results__.{io_type}`;ReAct 循环补完整支持(`call_external`/`call_service` + `collect`/`merge`/`has_fields`);若干死循环与陈旧测试修复。

> 注:0.3.0 内部未独立发布,本次合并为 v0.3.1 单版本。Git 历史从本仓首次提交开始,接续老仓 v0.2.4。

### 🆕 新增

- **TCB `TransitionResult::Ignored` 变体** ([transition.rs:53](evorule-tcb/src/transition.rs)):
  - 显式暴露"指令被忽略"(无匹配 transform 规则或规则产生 noop 效果)
  - 上层(reactor / cli)产生 Error 事实使系统显式感知,不再静默失败
- **ReAct 循环支持**(完整):
...
- **新域类型 `has_fields`** ([domain.rs](evorule-tcb/src/domain.rs)):
  - 检测对象字段集合,空 / null 数组视为 false
  - ReAct 循环 `if has_fields(tool_calls) then collect else io_request` 分支用
...
- **I/O 结果按类型隔离** (`__io_result__` → `__io_results__.{io_type}`):
  - 旧版单一字段会被任意 io_type 消费,可能让后续不同 io_type 指令错误走 on_true 消费旧值
  - v0.3.1 按 `__io_results__.{io_type}` 隔离,恢复执行后整体清除,杜绝残留
  - Invariant #2 + #4 同步更新
  - `inject_io_result` 签名增加 `io_type` 参数
- **`all([])` 兜底规则不再视为业务规则** ([transition.rs:179](evorule-tcb/src/transition.rs)):
  - 显式标注 `all` 兜底不算业务规则
  - 行为变化:未知指令不再静默当 noop,返回 `Ignored` → Error
- **死循环修复**:
  - null 注入不再触发 I/O 恢复执行(原 `exists` 域将 null 视为不存在,会无限重发 io_request)
  - error 响应也正确处理,不再重新推送原指令
- **CLI main.rs 重构**:
  - 从内联 `mod cli;` 改为 `use evorule_cli::cli`,统一通过 lib 暴露模块
  - `evorule` 二进制入口和命令完全兼容
- 版本同步 bump 至 0.3.1(workspace 单一真相源 + 各子 crate 依赖对齐)
...
- **Version 一致性**: Error 事实不 bump log version(原始设计)但 reactor 之前会 bump(不一致),已对齐 — Ignored reactor 处理现在也不 bump
- **测试与 v0.3.1 设计对齐**(2 个陈旧测试更新):
  - `evorule-cli::executor::test_execute_empty_core_eval_noop`: 旧期望 StateTransition,新期望 Error
  - `evorule-reactor::test_unknown_instruction_falls_to_noop`: 旧期望静默 noop,新验证产生 Error 事实

### 🆕 文档

- **doc-tests 补全**:
  - `evorule-governance::rule_validation::{validate_rules, validate_from_json}` 加示例
  - `evorule-cli::error::CliError::exit_code` 加错误码映射示例
...
- ✅ fact log 格式不变(cross-validate_with_tier1 / tier2 全过)
- ✅ CLI 命令与输出格式不变
