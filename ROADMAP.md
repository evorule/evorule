...
| 0.2.x | 机制层重构 + 移除策略层功能 + 形式化验证增强  | ✅ 已发布（v0.2.0 / v0.2.1 / v0.2.2）         |
| 0.3.x | 待规划                                        | 🔜 方向待定                                   |
...
| 阶段     | 时间点           | 目标                                                         |
| -------- | ---------------- | ------------------------------------------------------------ |
| 阶段 1   | 当前 → 1.0       | P0 属性 Kani 证明 + 差分测试 + T15 门控                      |
| 阶段 2   | 1.0 发布         | P0 全通过 + Coq 形式化 + TLA+ 全 tier + 自我审计             |
| 阶段 3   | 1.x 增强         | P1 全通过 + TLAPS 数学归纳 + cargo-fuzz                      |
| 阶段 4   | 学术增强（未来） | TLA+↔Rust 细化证明 + Coq 提取 + P2 属性                      |
| 阶段 5   | 第三方审计       | 前置：阶段 1-4 全完成 + 自我审计通过                         |

> 详见 [EVORULE_FORMAL_VERIFICATION_PLAN_v3.md §六](EVORULE_FORMAL_VERIFICATION_PLAN_v3.md#六分阶段路线图)。
