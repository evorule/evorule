...
> **快照**：v0.5.0（代码基线 `5fac8bd`；证据基线 `bdfb8d4`，2026-09-12；A 档 TCB Kani 证据基线随 Batch 1 重置为 `1c6ad84`（2026-09-13）、随 W3-1 再重置为 `1b340e5`（2026-09-14），见维护区）
...
| P0-3 | resolve_path 不 panic | tier0 | ✅ | proptest | A 档 11 个：`verify_resolve_path_simple_field` / `_nested_dot` / `_array_index` / `_double_dot` / `_escaped_dot` / `_empty_returns_none` / `_trailing_dot` / `_invalid_index_char` / `_missing_close_bracket` / `_deterministic` / `verify_array_index_bounds` | 11 份 `P0-3.<harness>_PASS_1b340e5_20260914_*`（evorule-tcb/verification/evidence/kani/；旧 `bdfb8d4`/`1c6ad84` 版已隔离 `_invalidated/` 批次 2/3） | W3-1 proof 源码变更（结构自检断言，CR-20260913-004 §3.8）后于 `1b340e5` 重跑 11/11 PASS（2026-09-14，WSL Kani 0.67.0，单 proof 0.3~4.0s）；kani.yml PR 闸门 | `1b340e5` 之前证据随 proof 源码变更依次失效隔离（M3.4：`bdfb8d4` 批次 2、`1c6ad84` 批次 3） |
