...
| 验证方法         | DO-178C Level A 要求 | Common Criteria EAL7 要求 | EvoRule 采用（对应位置） |
| ---------------- | ------------------- | ------------------------- | ------------------------ |
...
> **对标性质说明（不构成等级比较）**：上表仅列出 EvoRule 所用验证方法在 DO-178C / CC 中的**对应位置**，供专家判断方法论来源，**不构成等级或严格程度比较主张**。DO-178C Level A 与 CC EAL7 衡量的是「对整个开发过程的保证要求」（含流程、管理、工具资质、人员能力、独立评估）；EvoRule 的 AL0–AL4 衡量的是「单个命题的证据强度与验证对象同一性」——二者维度不同、不可换算（见 [ASSURANCE.md](../ASSURANCE.md) §9.2）。EvoRule **不主张达到或超越任何认证等级**（NC-15）。
