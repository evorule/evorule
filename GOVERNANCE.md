...
- CR 登记（`CHANGE_REQUEST.md`）是机制层变更的**强制治理流程**：变更实施前必须登记并获批准。自查校验由维护者本地 git pre-commit hook 执行——这是工程质量自查纪律，不是防伪造审查机制（TCB-2026-29 定性），不再作为 build.rs 公开门禁（既定裁定，2026-09-15）。
