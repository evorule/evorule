...
> evorule 仓当前处于 **0.1.x** 正式发布阶段（无预发布标识）。具体版本号见 `Cargo.toml`。
...
所有 tag 必须带 `v` 前缀：`v0.1.1`、`v0.2.0`、`v1.0.0`、`v1.0.0-rc.1`。
...
**A**: 0.x 阶段用户应**锁版本**（如 `evorule = "=0.1.1"`，**精确版本**）。`cargo update` 会自动跳到下一个 MINOR，这是用户预期的（因为 0.x 不承诺兼容）。
