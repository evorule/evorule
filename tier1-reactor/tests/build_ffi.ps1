...
# 检查输出文件
$DllPath = Join-Path $TargetDir "release\tier1_reactor.dll"
...
if ($Test) {
    ...
    if ($Compiler -eq "cl") {
        ...
        # GCC/MinGW 编译
        Push-Location $PSScriptRoot
        gcc -I"$IncludeDir" -L"$TargetDir\release" -ltier1_reactor -o test_evorule.exe test_evorule.c 2>&1
        Pop-Location
    }
