...
SERVER_BIN="$TARGET_DIR/tier2-governance-release/evorule-server"

# build-musl 不适用(tier2-governance 有 openssl-sys C 依赖),用普通 release
# 试过几次 tier2-governance 编译会卡 — 直接 build
log_path="/tmp/evorule_server_build.log"
echo "编译日志: $log_path"
echo "  (编译可能 5-10 分钟, 等待中...)"
if cargo build --release -p tier2-governance --bin evorule-server 2>&1 | tee "$log_path" | tail -5; then
...
RUST_LOG=info "$SERVER_BIN" \
    ...
    --core-eval "tier0-tcb/core_eval.json" \
...
ENDPOINTS=$(grep -cE '\.route\(' tier2-governance/src/api/server.rs 2>/dev/null || echo 0)
